...
import DymashevskijJumpAndBump.Drawable;
...
public class Level implements Drawable {
    ...
    public Level(String wallFile, String rabitFile) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            w = sc.nextInt();
            h = sc.nextInt();
            drawable.add(new Background(x, y, w, h, projFolder + sc.next()));
        ...
        for (int i = 0; i < n; i++) {
            ...
            y = sc.nextInt();
            sz = sc.nextInt();
            RabbitAI newRabbit = new RabbitAI(x, y, sz, sz, scoreXlength * (i + 1), (i + 1),
                    new File(projFolder + sc.next()), new File(projFolder + sc.next()),
                    new File(projFolder + sc.next()));
        ...
        n = sc.nextInt();
        for (int i = AIcount; i < n + AIcount; i++) {
            int x, y, sz;
            x = sc.nextInt();
            y = sc.nextInt();
            sz = sc.nextInt();
            Rabbit newRabbit = new Rabbit(x, y, sz, sz, scoreXlength * (i + 1), (i + 1),
                    new File(projFolder + sc.next()), new File(projFolder + sc.next()),
                    new File(projFolder + sc.next()));
            rabbits.add(newRabbit);
